...
<div class="breadcrumb">
    ...
    <ul>
        ...
        <li>View all banners locations</li>
    </ul>
</div>
@endsection
@section("content")
            ...
            <div class="row">
                <div class="col-md-12 text-right">
                    <a href="{{url('banners/create-locations')}}" class="btn btn-info">
                        <i class="nav-icon i-Add"></i>
                        <span class="item-name">Create Banner locations</span>
                    </a>
                </div>
                <div class="col-md-12">
                    <div class="card o-hidden mb-4">
                        <div class="card-header d-flex align-items-center">
                            <h3 class="w-50 float-left card-title m-0">All Banner locations</h3>
                                ...
                                <table id="user_table" class="table dataTable-collapse text-center">
                                    ...
                                    <tbody>
                                        @php
                                            $i= 1;
                                        @endphp
                                        @forelse ($allbannerlocations as $locations)
                                            
                                        <tr id="unis{{$locations->id}}">
                                            <th scope="row">{{$i++}}</th>                                            
                                            <td>{{$locations->name}}</td>
                                            <td>{{$locations->status}}</td>                                            
                                            <!-- <td>
                                                <a href="{{url('edit/faqs')}}/{{$locations->id}}" class="text-success mr-2">
                                                    <i class="nav-icon i-Pen-2 font-weight-bold"></i>
                                                </a>
                                                <a href="javascript:void(0)" class="delete_faqs" data-toggle="modal" data-target="#dnoeod1d11" onclick="delete_delta_step({{$locations->id}},'unis{{$locations->id}}');" id="faqs_delete"  class="text-danger mr-2">
                                        ...
                                        @empty
                                        <tr>
                                            <td colspan="50" class="text-center text-info">Banner locations not available</td>
                                        </tr>
                                        @endforelse                                        
                                    </tbody>
                                </table>
