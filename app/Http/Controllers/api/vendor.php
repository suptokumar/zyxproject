...
class vendor extends Controller
{
    function create_vendor(Request $request){
        if(!$request->name){
        return  json_encode(["status"=>201,"message"=>"Name Not Found"]);

        }
        if(!$request->email){
        return  json_encode(["status"=>201,"message"=>"Email Not Found"]);

        }
        if(!$request->password){
        return  json_encode(["status"=>201,"message"=>"Password Not Found"]);
        }
        if(uservendor::where("email",$request->email)->first()){
             return  json_encode(["status"=>201,"message"=>"Email already exists!"]);
        ...
        if($vendor->save()){
            return json_encode(["status"=>200,"message"=>"Vendor Created Successfuly","data"=>uservendor::find($vendor->id)]);
        }else{
            return  json_encode(["status"=>201,"message"=>"Failed to Create Vendor"]);
        }
    ...
    function get_vendor(Request $request)
    {
        ...
        return  json_encode(["status"=>201,"message"=>"`key` field is Required while using `handle`"]);

            }
        }else{
            if($request->key){
                return json_encode(uservendor::find($request->key));
            }else{
    ...
    function update_vendor(Request $request)
    {
        ...
        if($vendor->save()){
            return json_encode(["status"=>200,"message"=>"Vendor Saved Successfuly","data"=>uservendor::find($vendor->id)]);
        }else{
            return  json_encode(["status"=>201,"message"=>"Failed to Save Vendor"]);
        }

        }else{
            return  json_encode(["status"=>202,"message"=>"Id Not Matched"]);
        }


        }else{
            return  json_encode(["status"=>203,"message"=>"`id` not found"]);
        }
}
}
